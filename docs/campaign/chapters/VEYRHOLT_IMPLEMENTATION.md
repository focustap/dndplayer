...
- Civic Hall: formal chronological House Veyr owner portraits end in an unsettling theatrical Dealer portrait, with a heart-suit card and **HEARTS** in discoverable text. Automatic on inspection. No additional password fragment. One much older House Veyr painting should quietly contain **VII** carved into estate stone as optional background foreshadowing; do not highlight it during the HEARTS reveal unless players deliberately inspect or notice it.
...
- Direct Grand Floor → Final Table link. Where retained by the DM, optional Gallery, Private House and Foundations provide research, evidence and rest; never required to reach the boss. The live focused upgrade does not recreate deleted rooms. **Do not use Foundations for the VII reveal.** Preserve the Dealer profile, all twelve cards, restitution and Lornwatch hook.
- Final Table post-defeat: after the Dealer falls and the supernatural effects settle, reveal **VII** in the original stone directly beneath the central table. This is automatic, DM-controlled, requires no check, and should explicitly be identified as the same mark seen in Greymere. The carving must look older than the Dealer's occupation and must not explain the symbol's meaning.
...
- Verify the optional History Hall VII can remain unnoticed without blocking progression, and verify the Final Table VII is hidden before the Dealer's defeat and can be revealed reliably afterward without exposing it to players early.
...
## Issue 26 completion audit — September 22, 2026

The live campaign was audited from Greymere through the Final Table. The focused importer corrected Pell Aster's Objection clue, made RED automatically available, retired the Foundations VII reveal, updated the Final Table defeat instruction and added the optional History Hall plus guaranteed post-Dealer VII discoverables. Both VII images were uploaded through the authenticated R2 Worker and retain real campaign-scoped paths. A fresh post-import snapshot produced zero operations on rerun. Scenes, maps, characters, memberships, fog, overlays, tokens and combat sessions remained byte-for-byte unchanged; Hollow Dungeon 1 remained live.

A separate authenticated PLAYER session and the deployed OWNER session exercised the live Supabase/Realtime Dealer-card path. The player received deal and replay events, saw the flip animation and persistent card badge, retained state through refresh without replaying a stale animation, saw the Dealer's public card, and had no DM controls. Round expiration and redeal, Read the Back/reorder, Objection, Burn, Cut, Phase Two selection and exhaustion reshuffle were exercised. PLAYER RLS returned one public presentation, zero private deck rows, no draw order, and rejected a direct save attempt. Test character, monster and token HP, AC, speed, positions and conditions were unchanged.

The final live smoke test follows the playable route Greymere → East Road → city hub → all three automatic clues → spoken favorite-card gate → circus Jesters → Final Table → automatic VII reveal → restitution and Lornwatch hook. Generated test campaign rows and the temporary PLAYER account are removed after verification.
