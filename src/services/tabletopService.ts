...
export const refreshSignedAssetUrl = async (url: string): Promise<string | null> => {
  try {
    const parsed = new URL(url);
    if (parsed.pathname !== "/v1/object") return null;
    const campaignId = parsed.searchParams.get("campaignId")?.trim() ?? "";
    const path = parsed.searchParams.get("path")?.trim() ?? "";
    if (!campaignId || !path) return null;

    signedAssetCache.delete(path);
    signedAssetInflight.delete(path);
    ...
    const refreshed = await r2AssetService.sign(campaignId, path);
    if (!refreshed) return null;
    const value = {
      url: refreshed.url,
      expiresAt: Math.min(refreshed.expiresAt, Date.now() + SIGNED_ASSET_CACHE_MS),
    };
    signedAssetCache.set(path, value);
    persistSignedAsset(path, value);
