...
export const tabletopService = {
  async load(campaignId: string, playerView = false): Promise<TabletopState> {
    ...
    if (role === "OWNER" || role === "DM") {
      ...
      monsterInstances = ((data ?? []) as Record<string, unknown>[]).map((r) => { const t = r.monster_templates as Record<string, unknown>; const template: MonsterTemplate = { id: String(t.id), campaignId: String(t.campaign_id), name: String(t.name), imageUrl: t.image_url ? String(t.image_url) : null, maxHp: Number(t.max_hp), ac: Number(t.ac), speed: Number(t.speed), abilities: t.abilities as MonsterTemplate["abilities"], notes: String(t.notes ?? ""), traits: t.traits as MonsterTemplate["traits"], actions: t.actions as MonsterTemplate["actions"], bonusActions: t.bonus_actions as MonsterTemplate["bonusActions"], reactions: t.reactions as MonsterTemplate["reactions"] }; return { id: String(r.id), campaignId, templateId: String(r.template_id), customName: String(r.custom_name), currentHp: Number(r.current_hp), maxHp: Number(r.max_hp), ac: Number(r.ac), conditions: (r.conditions as string[]) ?? [], visible: Boolean(r.visible), notes: String(r.notes ?? ""), dead: Boolean(r.dead), template }; });
    }
    ...
    const characters: Character[] = ((characterRows ?? []) as Record<string, unknown>[]).map((r) => ({ id: String(r.id), campaignId: String(r.campaign_id), ownerId: String(r.owner_id), name: String(r.name), imageUrl: r.image_url ? String(r.image_url) : null, imagePath: r.image_path ? String(r.image_path) : null, currentHp: Number(r.current_hp), maxHp: Number(r.max_hp), ac: Number(r.ac), speed: Number(r.speed), passivePerception: Number(r.passive_perception), passiveInvestigation: Number(r.passive_investigation), passiveInsight: Number(r.passive_insight), notes: "", conditions: (r.conditions as string[]) ?? [] }));
    const monsterTemplates: MonsterTemplate[] = ((templateRows ?? []) as Record<string, unknown>[]).map((r) => ({ id: String(r.id), campaignId, name: String(r.name), imageUrl: r.image_url ? String(r.image_url) : null, imagePath: r.image_path ? String(r.image_path) : null, maxHp: Number(r.max_hp), ac: Number(r.ac), speed: Number(r.speed), abilities: r.abilities as MonsterTemplate["abilities"], notes: String(r.notes ?? ""), traits: r.traits as MonsterTemplate["traits"], actions: r.actions as MonsterTemplate["actions"], bonusActions: r.bonus_actions as MonsterTemplate["bonusActions"], reactions: r.reactions as MonsterTemplate["reactions"] }));
