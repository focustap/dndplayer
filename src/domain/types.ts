...
export interface Token { id: string; sceneId: string; referenceId: string | null; ownerUserId: string | null; type: TokenType; displayName: string; imageUrl: string | null; x: number; y: number; size: number; rotation: number; visible: boolean; locked: boolean; conditions: string[]; }
...
export interface MonsterTemplate { id: string; campaignId: string; name: string; imageUrl: string | null; maxHp: number; ac: number; speed: number; abilities: AbilityScores; notes: string; traits: MonsterAction[]; actions: MonsterAction[]; bonusActions: MonsterAction[]; reactions: MonsterAction[]; }
...
export interface Character { id: string; campaignId: string; ownerId: string; name: string; imageUrl: string | null; currentHp: number; maxHp: number; ac: number; speed: number; passivePerception: number; passiveInvestigation: number; passiveInsight: number; notes: string; conditions: string[]; }
...
export interface TabletopState { campaign: Campaign; role: CampaignRole; scene: Scene; overlays: SceneOverlay[]; tokens: Token[]; characters: Character[]; monsterInstances: MonsterInstance[]; combat: CombatSession; fogRegions: FogRegion[]; selectedTokenId: string | null; activeFogTool: FogTool | null; previewPlayerView: boolean; shiftIntel: boolean; connected: boolean; }
