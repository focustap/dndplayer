...
export type AttackPreset = "MELEE" | "RANGED" | "SPELL" | "SNEAK_ATTACK" | "SMITE" | "DRUID" | "WIZARD";
...
export interface SceneZoneMarker { id:string; campaignId:string; sceneId:string; label:string; x:number; y:number; radiusFt:number; color:string; opacity:number; visible:boolean; createdAt:string; updatedAt:string; }
export type SceneLinkAudioMode = "KEEP" | "PLAY" | "STOP";
export interface SceneLink { id: string; sceneId: string; destinationSceneId: string; label: string; x: number; y: number; musicMode: SceneLinkAudioMode; musicTrackId: string | null; musicLoop: boolean; musicNextTrackId: string | null; musicNextLoop: boolean; ambienceMode: SceneLinkAudioMode; ambienceTrackId: string | null; ambienceLoop: boolean; }
export interface SceneDiscoverable { id: string; campaignId: string; sceneId: string; name: string; storagePath?: string | null; imageUrl?: string | null; x: number; y: number; hidden: boolean; discoveredAt: string | null; discoveredBy: string | null; createdAt: string; }
export interface NpcTemplate { id:string; name:string; imageUrl:string|null; imagePath?:string|null; createdAt:string; updatedAt:string; }
...
export interface MonsterSave { ability: "STR" | "DEX" | "CON" | "INT" | "WIS" | "CHA"; dc: number; }
export interface MonsterSpellcasting { actionName?: string; ability: string | null; saveDc: number | null; attackBonus: number | null; spells: { frequency: string; spells: string[] }[]; }
export interface MonsterAction { name: string; description: string; kind?: "ACTION" | "MULTIATTACK"; usage?: MonsterUsage | null; attackType?: MonsterAttackType; attackBonus?: number | null; reach?: number | null; range?: { normal: number; long: number | null; unit: "ft" } | null; damage?: DamageComponent[]; save?: MonsterSave | null; conditions?: string[]; effects?: string[]; variants?: MonsterAction[]; spellcasting?: MonsterSpellcasting; multiattack?: { count?: number; options?: string; description: string }; }
export interface AbilityScores { str: number; dex: number; con: number; int: number; wis: number; cha: number; }
export interface MovementSpeeds { walk: number; fly: number; swim: number; climb: number; burrow: number; hover: boolean; }
export interface MonsterTemplate { id: string; name: string; imageUrl: string | null; imagePath?: string | null; creatureSize: string; creatureType: string; maxHp: number; hpFormula: string | null; ac: number; speed: number; movement: MovementSpeeds; initiative: { modifier: number | null; score: number | null }; abilities: AbilityScores; savingThrows: Record<string, number>; skills: Record<string, number>; damageVulnerabilities: string[]; damageResistances: string[]; damageImmunities: string[]; conditionImmunities: string[]; senses: { name: string; range?: number; unit?: string }[]; passivePerception: number | null; languages: string[]; notes: string; traits: MonsterAction[]; actions: MonsterAction[]; bonusActions: MonsterAction[]; reactions: MonsterAction[]; legendaryActions: MonsterAction[]; legendaryActionUses: number | null; spellcasting: MonsterSpellcasting[]; }
...
export interface Character { id: string; campaignId: string; ownerId: string; name: string; imageUrl: string | null; imagePath?: string | null; currentHp: number; maxHp: number; tempHp: number; ac: number; speed: number; abilities: AbilityScores; passivePerception: number; passiveInvestigation: number; passiveInsight: number; notes: string; conditions: string[]; }
...
export type Placement = { kind: "CHARACTER" | "MONSTER" | "NPC" | "DISCOVERABLE"; referenceId: string; name: string; imageUrl: string | null; } | { kind: "CHARACTERS"; referenceIds: string[]; name: string; imageUrl: null; } | { kind: "SCENE_LINK"; destinationSceneId: string; name: string; imageUrl: null; } | { kind:"ZONE_MARKER"; name:string; radiusFt:number; color:string; imageUrl:null; };
export interface AttackAnimationEvent { id: string; campaignId: string; attackerTokenId: string; targetTokenId: string; preset: AttackPreset; color:string|null; createdAt: string; }
export interface AttackSelection { attackerTokenId: string; preset: AttackPreset; color:string|null; }
...
export interface TabletopState { campaign: Campaign; role: CampaignRole; scene: Scene; scenes: Scene[]; sceneLinks: SceneLink[]; discoverables: SceneDiscoverable[]; treasure: SceneDiscoverable[]; discoveryReveal: SceneDiscoverable | null; overlays: SceneOverlay[]; zoneMarkers: SceneZoneMarker[]; tokens: Token[]; transientTokenIds: string[]; motionSegments: TokenMotionSegment[]; tokenInteractions: TokenInteraction[]; patrols: TokenPatrol[]; patrolEditTokenId: string | null; characters: Character[]; npcTemplates: NpcTemplate[]; monsterTemplates: MonsterTemplate[]; monsterInstances: MonsterInstance[]; combat: CombatSession; diceRolls: DiceRoll[]; selectedTokenId: string | null; activeInteractionTokenId: string | null; placement: Placement | null; attackSelection: AttackSelection | null; attackEvent: AttackAnimationEvent | null; cinematicEvent: CinematicEvent | null; dreadActive: boolean; previewPlayerView: boolean; shiftIntel: boolean; connected: boolean; }
