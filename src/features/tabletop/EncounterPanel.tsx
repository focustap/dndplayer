    ...
    <aside className="encounter-panel">
      <div className="panel-heading">
        <div>
          ...
          <h2>{state.combat.active ? "Active combat" : "Scene roster"}</h2>
        </div>
      </div>
      ...
      <div className="party-summary">
        <Users />
        <span>
          <b>{state.characters.length} heroes</b>
          <small>Characters available in this campaign</small>
        </span>
      </div>

      <p className="section-label">
        ON THE FIELD <span>{state.tokens.length}</span>
      </p>
      {state.tokens.map((token) => {
        const monster = state.monsterInstances.find(
        ...
        const detail = monster
          ...
          : character
            ? `${character.currentHp} / ${character.maxHp} HP · AC ${character.ac}`
            : token.type;
        const initiative =
          state.combat.entries.find(
            (entry) =>
              entry.tokenId === token.id ||
              entry.monsterInstanceId === token.referenceId,
          )?.initiative ?? "—";
      ...
      {dm && (
        <>
          ...
          <p className="placement-help">
            Choose a character, monster, or NPC, then click its position on the
            map. Press Esc to cancel.
          </p>

          <p className="section-label">
            CHARACTERS <span>{state.characters.length}</span>
          </p>
          ...
          <p className="section-label">
            MONSTER TEMPLATES <span>{state.monsterTemplates.length}</span>
          </p>
          {state.monsterTemplates.map((template) => (
            <button
              className="library-row"
              key={template.id}
              onClick={() =>
          ...
          <p className="section-label">
            NPCS <span>{state.npcTemplates.length}</span>
          </p>
          <div className="npc-create-row">
            ...
            <input
              ...
              onChange={(event) => {
                ...
                if (file) void createNpc(file);
                event.target.value = "";
              }}
            />
            <button
              disabled={!npcName.trim()}
              title="Create NPC with portrait"
              onClick={() => npcInput.current?.click()}
            >
              <Upload />
              Add
            </button>
          ...
          {state.npcTemplates.map((template) => (
            <div className="npc-library-entry" key={template.id}>
              <button
                className="library-row"
                onClick={() =>
                  actions.startPlacement({
              ...
              <button
                className="npc-delete"
                title={`Delete ${template.name}`}
                onClick={() => void actions.deleteNpcTemplate(template.id)}
              >
                <Trash2 />
              </button>
            </div>
          ...
          {state.placement && (
            ...
            >
              <Swords />
              PLACING · {state.placement.name} · ESC cancel
            </button>
          )}
        </>
      )}
