...
function InspectorContent({ tokenId }: { tokenId: string }) {
  ...
  return <aside className="inspector-panel">
    ...
    {dm && <label className="form-field" style={{margin:"0 0 13px"}}>
      <span>TOKEN SIZE ON MAP</span>
      <select
        value={String(token.size)}
        onChange={(event) => void actions.patchToken(token.id, { size: Number(event.target.value) })}
        style={{width:"100%",padding:"9px",border:"1px solid #39433e",background:"#0f1311",color:"#eee"}}
      >
      ...
      <small>Changes the token footprint without changing the creature stat block.</small>
    </label>}
