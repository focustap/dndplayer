...
import type { FogRegion, FogTool, Scene, SceneOverlay, Token } from "../../domain/types";

export interface EngineSnapshot { scene: Scene; overlays: SceneOverlay[]; tokens: Token[]; fogRegions: FogRegion[]; canDm: boolean; playerView: boolean; shiftIntel: boolean; activeFogTool: FogTool | null; selectedTokenId: string | null; monsterIntel: Record<string, { hp: number; maxHp: number; ac: number }>; canMove(token: Token): boolean; }
...
export class SceneEngine {
  ...
  async init() {
    ...
    this.app.canvas.addEventListener("wheel", this.handleWheel, { passive: false }); this.center(); if (this.snapshot) void this.render(this.snapshot);
  }
  ...
  async render(snapshot: EngineSnapshot) {
    this.snapshot = snapshot; if (!this.app.renderer) return; const version = ++this.renderVersion; this.root.removeChildren();
  ...
  private async renderOverlay(overlay: SceneOverlay, canDm: boolean) {
    ...
    display.position.set(overlay.x,overlay.y); display.rotation=overlay.rotation; display.alpha=overlay.visible?overlay.opacity:(canDm?.3:0); display.zIndex=100+overlay.zIndex; display.eventMode=canDm&&!overlay.locked?"static":"none"; display.cursor="move"; display.on("pointerdown",(e: FederatedPointerEvent)=>{ e.stopPropagation(); const p=this.root.toLocal(e.global); this.dragging={kind:"OVERLAY",id:overlay.id,dx:p.x-overlay.x,dy:p.y-overlay.y}; }); this.root.addChild(display);
  ...
  private async renderToken(token: Token, snapshot: EngineSnapshot) {
    ...
    c.on("pointertap",(e: FederatedPointerEvent)=>{e.stopPropagation();this.callbacks.onSelect(token.id);}); c.on("rightclick",(e: FederatedPointerEvent)=>{e.stopPropagation();this.callbacks.onContext(token.id,e.clientX,e.clientY);}); c.on("pointerdown",(e: FederatedPointerEvent)=>{if(!snapshot.canMove(token))return;e.stopPropagation();const p=this.root.toLocal(e.global);this.dragging={kind:"TOKEN",id:token.id,dx:p.x-token.x,dy:p.y-token.y};}); this.root.addChild(c);
  }
  ...
  private handleStageDown(e:FederatedPointerEvent){if(this.spaceDown||e.button===1){this.panning=true;this.panStart={x:e.global.x,y:e.global.y,rootX:this.root.x,rootY:this.root.y};return;}if(this.snapshot?.canDm&&this.snapshot.activeFogTool){const p=this.root.toLocal(e.global);this.fogDraft={tool:this.snapshot.activeFogTool,sx:p.x,sy:p.y,x:p.x,y:p.y,samples:[p.x,p.y]};return;}this.callbacks.onSelect(null);}
