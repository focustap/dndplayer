...
import type { AttackAnimationEvent, AttackSelection, FogTool, Placement, Scene, SceneLink, SceneOverlay, Token } from "../../domain/types";
...
export class SceneEngine {
  ...
  private handleUp(){if(this.fogDraft){const d=this.fogDraft;if(d.tool.endsWith("RECT")){const x=Math.min(d.sx,d.x),y=Math.min(d.sy,d.y);this.callbacks.onFogCommit(d.tool,[x,y,Math.max(10,Math.abs(d.x-d.sx)),Math.max(10,Math.abs(d.y-d.sy))]);}else this.callbacks.onFogCommit(d.tool,[(this.snapshot?.scene.gridSize??80)*.65,...d.samples]);this.fogDraft=null;return;}const candidate=this.pointerCandidate;this.pointerCandidate=null;if(candidate){if(candidate.kind==="TOKEN")this.callbacks.onSelect(candidate.id);this.panning=false;return;}if(!this.dragging){this.panning=false;return;}const drag=this.dragging;const changed=Math.hypot(drag.x-drag.originX,drag.y-drag.originY)>.01;if(drag.kind==="TOKEN"){if(changed){this.instrumentation.moveCommits++;this.debug("token move commit");this.callbacks.onMoveCommit(drag.id,drag.x,drag.y);}else this.callbacks.onSelect(drag.id);}else if(drag.kind==="OVERLAY"&&changed){this.instrumentation.overlayCommits++;this.debug("overlay move commit");this.callbacks.onOverlayCommit(drag.id,drag.x,drag.y);}else if(drag.kind==="SCENE_LINK"&&changed){this.instrumentation.sceneLinkCommits++;this.debug("scene link move commit");this.callbacks.onSceneLinkCommit(drag.id,drag.x,drag.y);}this.dragging=null;this.panning=false;}
