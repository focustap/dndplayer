...
export class SceneEngine {
  ...
  private applyPlacement(snapshot: EngineSnapshot | null) { if (!snapshot) return; const placement=snapshot.placement; const identity=!placement?null:"referenceId" in placement?placement.referenceId:"destinationSceneId" in placement?placement.destinationSceneId:"referenceIds" in placement?placement.referenceIds.join(","):placement.kind==="ZONE_MARKER"?`${placement.radiusFt}:${placement.color}`:placement.name;const key=placement?`${placement.kind}:${identity}:${placement.name}:${snapshot.scene.gridSize}:${snapshot.scene.feetPerCell}`:null; if (key===this.placementKey) return; this.placementGhost?.destroy({children:true}); this.placementGhost=null; this.placementKey=key; if (placement) this.renderPlacementGhost(placement,snapshot.scene.gridSize); }
