...
import type { AttackAnimationEvent, AttackSelection, CinematicEvent, FogTool, Placement, Scene, SceneDiscoverable, SceneLink, SceneOverlay, Token, TokenPatrol } from "../../domain/types";
import { createSceneStructureKey, type SceneStructureSnapshot } from "./sceneStructureKey";

export interface EngineSnapshot extends SceneStructureSnapshot { builder: boolean; shiftIntel: boolean; activeFogTool: FogTool | null; placement: Placement | null; patrolEdit: TokenPatrol | null; attackSelection: AttackSelection | null; attackEvent: AttackAnimationEvent | null; cinematicEvent: CinematicEvent | null; selectedTokenId: string | null; transientTokenIds: string[]; monsterIntel: Record<string, { hp: number; maxHp: number; ac: number }>; canMove(token: Token): boolean; }
interface EngineCallbacks { onSelect(id: string | null): void; onMoveCommit(id: string, x: number, y: number): void; onTokenMove(id:string,x:number,y:number,final:boolean):void; onOverlayCommit(id: string, x: number, y: number): void; onSceneLinkCommit(id: string, x: number, y: number): void; onDiscoverableCommit(id:string,x:number,y:number):void; onPatrolWaypointAdd(x:number,y:number):void; onPatrolWaypointMove(id:string,x:number,y:number):void; onPatrolWaypointDelete(id:string):void; onSceneLinkActivate(id: string): void; onDiscover(id:string):void; onFogCommit(tool: FogTool, points: number[]): void; onPlace(x: number, y: number): void; onAttackTarget(id: string | null): void; onContext(id: string, x: number, y: number): void; }
...
export class SceneEngine {
  ...
  async render(snapshot: EngineSnapshot) {
    ...
    this.renderLighting(snapshot.scene); if (snapshot.scene.fogEnabled) this.renderFog(snapshot); this.renderPatrolPath(snapshot); this.applyPositions(this.snapshot); this.applyDiscoverablePositions(this.snapshot); this.applyIntel(this.snapshot); this.applySelection(this.snapshot); this.applyPlacement(this.snapshot); this.applyAttackEvent(this.snapshot);
  }
  ...
  private handleStageDown(e:FederatedPointerEvent){if(this.spaceDown||e.button===1){this.panning=true;this.panStart={x:e.global.x,y:e.global.y,rootX:this.root.x,rootY:this.root.y};return;}if(this.snapshot?.attackSelection){this.callbacks.onAttackTarget(null);return;}if(this.snapshot?.patrolEdit&&this.snapshot.builder&&this.snapshot.canDm){const p=this.root.toLocal(e.global);this.callbacks.onPatrolWaypointAdd(p.x,p.y);return;}if(this.snapshot?.placement&&this.snapshot.canDm){const p=this.root.toLocal(e.global);this.callbacks.onPlace(p.x,p.y);return;}if(this.snapshot?.canDm&&this.snapshot.activeFogTool){const p=this.root.toLocal(e.global);this.fogDraft={tool:this.snapshot.activeFogTool,sx:p.x,sy:p.y,x:p.x,y:p.y,samples:[p.x,p.y]};return;}this.callbacks.onSelect(null);}
  ...
  private handleUp(){if(this.fogDraft){const d=this.fogDraft;if(d.tool.endsWith("RECT")){const x=Math.min(d.sx,d.x),y=Math.min(d.sy,d.y);this.callbacks.onFogCommit(d.tool,[x,y,Math.max(10,Math.abs(d.x-d.sx)),Math.max(10,Math.abs(d.y-d.sy))]);}else this.callbacks.onFogCommit(d.tool,[(this.snapshot?.scene.gridSize??80)*.65,...d.samples]);this.fogDraft=null;return;}const candidate=this.pointerCandidate;this.pointerCandidate=null;if(candidate){if(candidate.kind==="TOKEN")this.callbacks.onSelect(candidate.id);this.panning=false;return;}if(!this.dragging){this.panning=false;return;}const drag=this.dragging;const changed=Math.hypot(drag.x-drag.originX,drag.y-drag.originY)>.01;if(drag.kind==="TOKEN"){if(changed){this.instrumentation.moveCommits++;this.debug("token move commit");this.callbacks.onTokenMove(drag.id,drag.x,drag.y,true);this.callbacks.onMoveCommit(drag.id,drag.x,drag.y);}else this.callbacks.onSelect(drag.id);}else if(drag.kind==="OVERLAY"&&changed){this.instrumentation.overlayCommits++;this.debug("overlay move commit");this.callbacks.onOverlayCommit(drag.id,drag.x,drag.y);}else if(drag.kind==="SCENE_LINK"&&changed){this.instrumentation.sceneLinkCommits++;this.debug("scene link move commit");this.callbacks.onSceneLinkCommit(drag.id,drag.x,drag.y);}else if(drag.kind==="DISCOVERABLE"&&changed){this.callbacks.onDiscoverableCommit(drag.id,drag.x,drag.y);}else if(drag.kind==="PATROL_WAYPOINT"&&changed){this.callbacks.onPatrolWaypointMove(drag.id,drag.x,drag.y);}this.dragging=null;this.panning=false;}
