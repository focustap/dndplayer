...
interface EngineCallbacks { onSelect(id: string | null): void; onMoveCommit(id: string, x: number, y: number): void; onTokenMove(id:string,x:number,y:number,final:boolean):void; onOverlayCommit(id: string, x: number, y: number): void; onSceneLinkCommit(id: string, x: number, y: number): void; onDiscoverableCommit(id:string,x:number,y:number):void; onSceneLinkActivate(id: string): void; onDiscover(id:string):void; onFogCommit(tool: FogTool, points: number[]): void; onPlace(x: number, y: number): void; onAttackTarget(id: string | null): void; onContext(id: string, x: number, y: number): void; }
...
export class SceneEngine {
  ...
  async render(snapshot: EngineSnapshot) {
    this.snapshot = snapshot; if (!this.app.renderer) return; const structureKey=createSceneStructureKey(snapshot); if (structureKey===this.structureKey) { this.applyPositions(snapshot); this.applyDiscoverablePositions(snapshot); this.applyIntel(snapshot); this.applySelection(snapshot); this.applyPlacement(snapshot); this.applyAttackEvent(snapshot); return; } this.structureKey=structureKey; const version = ++this.renderVersion; this.instrumentation.structuralRebuilds++; this.debug("structural rebuild"); this.clearCinematic(); this.clearAttackEffects(); this.root.removeChildren(); this.tokenDisplays.clear(); this.overlayDisplays.clear(); this.sceneLinkDisplays.clear(); this.discoverableDisplays.clear(); this.tokenPositionTargets.clear(); this.tokenMovementAnimations.clear(); this.selectedRings.clear(); this.intelDisplays.clear(); this.intelKey=null; this.selectedTokenId=null; this.placementGhost=null; this.placementKey=null;
    ...
    this.renderLighting(snapshot.scene); if (snapshot.scene.fogEnabled) this.renderFog(snapshot); this.applyPositions(this.snapshot); this.applyDiscoverablePositions(this.snapshot); this.applyIntel(this.snapshot); this.applySelection(this.snapshot); this.applyPlacement(this.snapshot); this.applyAttackEvent(this.snapshot);
  }
