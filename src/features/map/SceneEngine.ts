...
export class SceneEngine {
  ...
  async render(snapshot: EngineSnapshot) {
    ...
    if (snapshot.scene.fogEnabled) this.renderFog(snapshot); this.applyPositions(this.snapshot); this.applyIntel(this.snapshot); this.applySelection(this.snapshot); this.applyPlacement(this.snapshot); this.applyAttackEvent(this.snapshot);
  }
  ...
  private async renderToken(token: Token, snapshot: EngineSnapshot) {
    const cell=snapshot.scene.gridSize; const radius=cell*.36*token.size; const c=new Container(); c.position.set(token.x,token.y); c.rotation=token.rotation; c.zIndex=300; c.eventMode="static"; c.cursor=snapshot.canMove(token)?"grab":"pointer";
    ...
    c.on("rightclick",(e: FederatedPointerEvent)=>{e.stopPropagation();this.callbacks.onContext(token.id,e.clientX,e.clientY);}); c.on("pointerdown",(e: FederatedPointerEvent)=>{if(e.button!==0)return;if(this.snapshot?.attackSelection){e.stopPropagation();this.callbacks.onAttackTarget(token.id);return;}if(!this.snapshot?.canMove(token))return;e.stopPropagation();const p=this.root.toLocal(e.global);const currentX=c.position.x,currentY=c.position.y;this.pointerCandidate={kind:"TOKEN",id:token.id,startX:e.global.x,startY:e.global.y,originX:currentX,originY:currentY,dx:p.x-currentX,dy:p.y-currentY,display:c};}); this.tokenDisplays.set(token.id,c); this.root.addChild(c);
  }
