-- Scene-link audio cues, music queueing, and a separate ambience channel.

alter table public.campaign_audio_state
  add column next_track_id uuid references public.campaign_audio_tracks(id) on delete set null,
  add column next_loop boolean not null default true;
...
alter table public.scene_links
  ...
  add column music_next_track_id uuid references public.campaign_audio_tracks(id) on delete set null,
  add column music_next_loop boolean not null default true,
...
begin
  ...
  if v_action = 'PLAY' then
    ...
    v_switching := p_track_id is not null and p_track_id is distinct from v_state.track_id;
    if v_switching then
      v_position := 0;
    end if;
    ...
    set track_id = v_target_track,
        ...
        started_at = v_now,
        next_track_id = case when v_switching then null else next_track_id end,
        next_loop = case when v_switching then true else next_loop end,
        revision = revision + 1,
...
create or replace function public.queue_campaign_audio(
  p_campaign_id uuid,
  p_next_track_id uuid default null,
  p_next_loop boolean default true
)
returns public.campaign_audio_state
language plpgsql
security definer
...
begin
  if not exists (
    ...
    raise exception 'Only a campaign DM can queue music.';
  end if;

  if p_next_track_id is not null and not exists (
    select 1
    from public.campaign_audio_tracks t
    where t.id = p_next_track_id
      and t.campaign_id = p_campaign_id
  ) then
    raise exception 'That queued track does not belong to this campaign.';
  end if;
  ...
  update public.campaign_audio_state
  set next_track_id = p_next_track_id,
      next_loop = coalesce(p_next_loop, true),
      revision = revision + 1,
      updated_by = (select auth.uid()),
...
revoke all on function public.queue_campaign_audio(uuid,uuid,boolean) from public, anon;
grant execute on function public.queue_campaign_audio(uuid,uuid,boolean) to authenticated;

create or replace function public.advance_campaign_audio(p_campaign_id uuid)
returns public.campaign_audio_state
language plpgsql
security definer
set search_path = ''
...
begin
  if not exists (
    ...
    raise exception 'Only a campaign DM can advance music.';
  end if;
  ...
  if v_state.next_track_id is not null then
    update public.campaign_audio_state
    set track_id = v_state.next_track_id,
        playing = true,
        loop = v_state.next_loop,
        position_ms = 0,
        started_at = v_now,
        next_track_id = null,
        next_loop = true,
...
revoke all on function public.advance_campaign_audio(uuid) from public, anon;
grant execute on function public.advance_campaign_audio(uuid) to authenticated;
...
begin
  ...
  if v_action = 'PLAY' then
    ...
    if not exists (
      ...
      raise exception 'That ambience track does not belong to this campaign.';
    end if;
...
begin
  ...
  if v_link.music_track_id is not null and not exists (
    ...
    raise exception 'Scene link music track is invalid';
  end if;

  if v_link.music_next_track_id is not null and not exists (
    select 1 from public.campaign_audio_tracks t
  ...
  if v_link.ambience_track_id is not null and not exists (
    select 1 from public.campaign_audio_tracks t
    where t.id = v_link.ambience_track_id and t.campaign_id = v_campaign_id
  ) then
    raise exception 'Scene link ambience track is invalid';
  end if;

  if v_link.music_mode = 'PLAY' then
    perform public.control_campaign_audio(v_campaign_id, 'PLAY', v_link.music_track_id, v_link.music_loop);
    perform public.queue_campaign_audio(v_campaign_id, v_link.music_next_track_id, v_link.music_next_loop);
  ...
  if v_link.ambience_mode = 'PLAY' then
    perform public.control_campaign_ambience(v_campaign_id, 'PLAY', v_link.ambience_track_id, v_link.ambience_loop);
