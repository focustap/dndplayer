-- A slower default makes ordinary map distances visibly glide while retaining
-- the DM speed control. Existing untouched default patrols receive the same
-- improved baseline.
alter table public.token_patrols alter column speed set default 60;
update public.token_patrols set speed = 60 where speed = 105;

-- Retire the completed segment inside the same transaction as its persisted
-- token/patrol checkpoint. A reload can therefore never rehydrate an already
-- completed segment as active.
...
begin
  ...
  where token_id = v_token.id and active;
